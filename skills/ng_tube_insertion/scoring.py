"""
ng_tube_insertion Scoring Plugin
"""
import numpy as np
...
class SkillScorer:
    def __init__(self, config=None):
        ...
        self.skill_name = "ng_tube_insertion"
    ...
    def extract_features(self, landmarks):
        ...
        lw=self._lm(landmarks,"left_wrist"); le=self._lm(landmarks,"left_elbow")
        re=self._lm(landmarks,"right_elbow"); ls=self._lm(landmarks,"left_shoulder")
        rs=self._lm(landmarks,"right_shoulder"); lh=self._lm(landmarks,"left_hip")
        rh=self._lm(landmarks,"right_hip")
        f["nose_distance"]=float(np.sqrt((rw[0]-nose[0])**2+(rw[1]-nose[1])**2))
        ...
        if "dom_y" in self.landmark_history and len(self.landmark_history["dom_y"])>=5:
            f["depth_movement"]=abs(self.landmark_history["dom_y"][-1]-self.landmark_history["dom_y"][-5])
    ...
    def compute_metrics(self, features):
        ...
        nd=features.get("nose_distance",0.3)
        m["angle_score"]=round(85 if 0.1<=nd<=0.4 else 65,1)
        ia=features.get("insertion_angle",80)
        dev=abs(ia-90)
        m["angle_score"]=round(max(40,100-dev*1.2),1)
        m["stability"]=round(max(40,100-features.get("hand_stability",0)*30),1)
        dm=features.get("depth_movement",0.03)
        m["technique"]=round(85 if 0.01<=dm<=0.05 else 70,1)
        m["smoothness"]=round(80,1)
    ...
    def get_feedback(self, metrics, features):
        ...
        ia=features.get("insertion_angle",80)
        if ia<70 or ia>110: fb.append({"type":"critical","message":"插入角度偏离90度","severity":"critical","metric":"angle_score"})
        if features.get("hand_stability",0)>1.5: fb.append({"type":"warning","message":"手部不稳","severity":"warning","metric":"stability"})
