...
class SkillScorer:
    ...
    def extract_features(self, landmarks):
        ...
        lw=self._lm(landmarks,"left_wrist"); ls=self._lm(landmarks,"left_shoulder")
        rs=self._lm(landmarks,"right_shoulder"); le=self._lm(landmarks,"left_elbow")
        ...
        if "dom_y" in self.landmark_history and len(self.landmark_history["dom_y"])>=5:
            f["insertion_depth"]=abs(self.landmark_history["dom_y"][-1]-self.landmark_history["dom_y"][-5])
        if "right_wrist" in self.landmark_history:
            ...
            if len(rt)>=5:
                ...
                f["rotational_motion"]=float(1.0/(1.0+np.var(dp)*20)) if dp else 0.7
    ...
    def compute_metrics(self, features):
        ...
        m["positioning"]=round(90 if 0.1<=hfd<=0.3 else 70,1)
        m["stability"]=round(max(40,100-features.get("hand_stability",0)*30),1)
        dc=features.get("insertion_depth",0.03)
        m["technique"]=round(85 if 0.01<=dc<=0.05 else (60 if dc<0.01 else 75),1)
        m["smoothness"]=round(features.get("rotational_motion",0.7)*100,1)
    ...
    def get_feedback(self, metrics, features):
        ...
        hfd=features.get("hand_face_distance",0.3)
        if hfd>0.4: fb.append({"type":"info","message":"插入位置需靠近鼻孔","severity":"info","metric":"positioning"})
        if features.get("hand_stability",0)>1.5: fb.append({"type":"warning","message":"手部不稳，可能损伤气道","severity":"warning","metric":"stability"})
