...
class SkillScorer:
    ...
    def extract_features(self, landmarks):
        ...
        f["lw_x"]=lw[0]; f["lw_y"]=lw[1]; f["rw_x"]=rw[0]; f["rw_y"]=rw[1]
        if "lw_x" in self.landmark_history and len(self.landmark_history["lw_x"])>=30:
            xh=self.landmark_history["lw_x"][-30:]; yh=self.landmark_history["lw_y"][-30:]
            f["hand_mobility"]=float((max(xh)-min(xh))*(max(yh)-min(yh)))
        if "lw_y" in self.landmark_history and len(self.landmark_history["lw_y"])>=30:
            ...
            f["movement_pattern"]=min(1.0,zc/8) if zc>0 else 0
    ...
    def compute_metrics(self, features):
        ...
        hm=features.get("hand_mobility",0.2)
        m["coverage"]=round(85 if 0.05<=hm<=0.6 else 65,1)
        m["pattern"]=round(features.get("movement_pattern",0.7)*100,1)
    ...
    def get_feedback(self, metrics, features):
        ...
        if features.get("hand_mobility",0.2)<0.05: fb.append({"type":"info","message":"检查活动范围偏小","severity":"info","metric":"coverage"})
        if features.get("movement_pattern",0.7)<0.5: fb.append({"type":"warning","message":"检查动作缺少规律性","severity":"warning","metric":"pattern"})
